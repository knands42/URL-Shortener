...
INSERT INTO shortened_urls (
  ...
  $1, $2
) RETURNING id, original_url, hash, number_of_accesses, created_at, updated_at
`
...
SELECT id, original_url, hash, number_of_accesses, created_at, updated_at FROM shortened_urls WHERE hash = $1
`
...
SELECT id, original_url, hash, number_of_accesses, created_at, updated_at FROM shortened_urls WHERE original_url = $1
`
...
const increaseNumberOfAccesses = `-- name: IncreaseNumberOfAccesses :exec
UPDATE shortened_urls SET number_of_accesses = number_of_accesses + 1 WHERE hash = $1
`

func (q *Queries) IncreaseNumberOfAccesses(ctx context.Context, hash string) error {
	_, err := q.db.Exec(ctx, increaseNumberOfAccesses, hash)
	return err
}
